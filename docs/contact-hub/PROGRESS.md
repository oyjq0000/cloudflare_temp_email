...
- Local pre-push evidence at documentation time: Worker 54/54 unit tests + lint + dry-run build passed; Frontend 69/69 unit tests + production build passed earlier in the same clean clone; Compose config and Playwright discovery pass with 206 tests in 53 files. Full Docker and GitHub Actions evidence is recorded only after it actually runs in `RC_HARDENING_REPORT.md`.
