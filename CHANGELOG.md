...
- test: |Contact Hub| 使用 HTTP mock 覆盖 Resend/Brevo Message ID 与错误分类，覆盖缺 Secret 零调用、Provider 配置脱敏/显式绑定，并增加 Mailpit SMTP 投递用例
- test: |Contact Hub| 覆盖并发双击只发送一次、成功/失败/未知映射、Failed Retry、Unknown 禁止 Retry/关联 Force Resend、Reply-To/线程头、跨 Domain From 拒绝和 CRLF 防护
